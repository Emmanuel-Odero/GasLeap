...
use gasleap_runtime::{opaque::Block, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		gasleap_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		gasleap_runtime::native_version()
	}
}

pub type ParachainExecutor = NativeElseWasmExecutor<ExecutorDispatch>;
...
async fn start_node_impl(
	...
	.await
	.map_err(|e| sc_service::Error::Other(format!("Could not build the relay chain interface: {}", e)))?;
	...
	let overseer_handle = relay_chain_interface
		.overseer_handle()
		.map_err(|e| sc_service::Error::Other(format!("Failed to get overseer handle: {}", e)))?;
	...
	if validator {
		start_collator(StartCollatorParams {
			...
			parachain_consensus: build_consensus(
				client.clone(),
				block_import,
...
/// Build the consensus algorithm for the parachain.
fn build_consensus(
	client: Arc<ParachainClient>,
	block_import: ParachainBlockImport,
...
struct OffchainTransactionPoolFactory<PoolApi> {
	pool: Arc<PoolApi>,
}

impl<PoolApi> OffchainTransactionPoolFactory<PoolApi> {
	pub fn new(pool: Arc<PoolApi>) -> Self {
		Self { pool }
	}
}

impl<PoolApi> sc_transaction_pool_api::OffchainTransactionPoolFactory<Block>
	for OffchainTransactionPoolFactory<PoolApi>
where
	PoolApi: sc_transaction_pool_api::TransactionPool<Block = Block> + 'static,
{
	type OffchainTransactionPool = Arc<PoolApi>;

	fn create_transaction_pool(
		&self,
		_best_block_hash: <Block as sp_runtime::traits::Block>::Hash,
	) -> Self::OffchainTransactionPool {
		self.pool.clone()
	}
}
...
use polkadot_service::OverseerHandle;
use sc_basic_authorship::Proposer;
